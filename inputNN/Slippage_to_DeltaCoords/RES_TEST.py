...
cols_to_use = ["m1vel", "m2vel", "m3vel", "surf", "m1cur", "m2cur", "m3cur", "w1slip", "w2slip", "w3slip", "vx", "vy", "omega"]
...
val_raw, test_raw = train_test_split(
    ...
    random_state=42, 
    shuffle=True, 
    stratify=temp_raw["surf"]
)

train = pd.get_dummies(train_raw, columns=["surf"], prefix="type", prefix_sep="_").astype(float)
val = pd.get_dummies(val_raw, columns=["surf"], prefix="type", prefix_sep="_").astype(float)
test = pd.get_dummies(test_raw, columns=["surf"], prefix="type", prefix_sep="_").astype(float)
...
features = ["m1vel", "m2vel", "m3vel", "w1slip", "w2slip", "w3slip", "type_brown", "type_gray", "type_green", "type_table"]
targets = ["vx", "vy", "omega"]
...
st = torch.load(r"C:\Users\User\Documents\MyPythonProjects\inputNN\Slippage_to_DeltaCoords\MLP_RES_study_20260524_104825\MLPconfig.pth")
model.load_state_dict(st["model"])

data = {"train": train_loader, "val": val_loader, "test": test_loader}

for key, loader in data.items():
    res = model.evaluate(loader, name=key, save_path=root_path, device=device)
    metrics_df = pd.DataFrame(res, index=["Дельта Х", "Дельта У", "Дельта Фи"]).T

    metrics_df.to_csv(os.path.join(root_path, 
                                    f"FINAL_metrics_{key}.csv"), 
                                    index_label="Метрики", 
                                    encoding="utf-8-sig")
