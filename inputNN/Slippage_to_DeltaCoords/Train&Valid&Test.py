...
if __name__ == "__main__":
    ...
    train = pd.get_dummies(train_raw, columns=["surf"], prefix="type_").astype(float)
    val = pd.get_dummies(val_raw, columns=["surf"], prefix="type_").astype(float)
    test = pd.get_dummies(test_raw, columns=["surf"], prefix="type_").astype(float)
    ...
    for features in features_variations:
        ...
        print(f"Подбор лучших параметров MLP для набора фичей {', '.join(features)} для таргетов {', '.join(targets)}")
        ...
        for key, loader in loaders.items():
            ...
            metrics_df.to_csv(os.path.join(current_feat_path, 
